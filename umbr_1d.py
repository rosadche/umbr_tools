...
class umbrella_set(object):
        ...
        def umbr_harm(self, min_val, max_val, num_im, endpoint, k):
            ...
            self.num_im = num_im
            self.umbr_harm_locs = np.linspace(min_val, max_val, num=num_im, endpoint=endpoint)
...
class adaptive_umbrella_collection(object):
        ...
        def __init__(self, dict_umbrella_sets):
            ...
            if not isinstance(dict_umbrella_sets, dict):
                raise Exception('Collection of umbrella sets must be provided as dictionary!')
            else:
                ...
                self.K_per_set   = np.asarray( [self.dict_umbr_sets[x].num_im for x in self.umbr_sets_keys] ) # many harmonic umbrellas are in each set for all sets
                self.K           = np.sum( self.K_per_set )                                                   # K is total number of images. 
                self.K_digitize  = np.digitize(np.arange(0, self.K), np.cumsum(self.K_per_set))               # which set does every image belong to
                
                # pull initial temperatures from umbrella sets
                self.T_k     = np.concatenate( [self.dict_umbr_sets[x].temps for x in self.umbr_sets_keys] )
        ...
        def get_plumed_external_biases(self):
            ...
            plumed_txt = ""
            plumed_txt += f"{self.cv_col}: READ FILE={colvar_file} VALUES={self.cv_col} IGNORE_TIME IGNORE_FORCES" + "\n"
            ...
            for m in range(len(self.umbr_sets_keys)):
                ...
                if this_umbr_set.umbr_static_file != None:
                    ...
                    plumed_txt += f"ext{m}: EXTERNAL ARG={self.cv_col} FILE={copied_name} SCALE={this_umbr_set.static_scale}" + "\n"
        ...
        def do_mbar(self, umbr_options=None, mbar_options=None):
            ...
            needed_umbr_options = ("cv_col", "N_max", "nbins", "outtemp", "pot_ener_col", "g", "verbose", "outfile", "units", "kB")
            
            if umbr_options == None: umbr_options = dict()
            for option in needed_umbr_options:
                if option not in needed_umbr_options: umbr_options[option] = None
            ...
            if (self.pot_ener_col == None) and not (np.all(self.T_k == self.T_k[0])):
                raise Exception('A column containing potential energy must be given when reweighting with simulations at different temperatures!')
            ...
            if g != None:
                if isinstance(g, list) or isinstance(g, np.ndarray):
                    if len(g) != self.K:
                        raise Exception('g as an array must be list/array of integers with len of total windows!')
                    else:
                        self.g_k[:] = g
                else:
                    if not isinstance(g, int):
                        raise Exception('g must bean integer, an list/array of integers, or None')
                    else:
                        self.g_k[:] = g
            
            
            #------ Allocate Storage --------------------------------------------------------------
            self.N_k = np.zeros([self.K], dtype=int)      # N_k[k] is the number of snapshots from umbrella simulation k
            
            self.restraint_k = np.zeros([self.K,2])       # Retraint_k[k] is the Umbrella spring constant and center vals for simualtion k: r1, k1
            
            self.cv_mat_kn = np.zeros([self.K, N_max])    # cv_mat_kn[k,n] is the CV value for snapshot n from umbrella simulation k
            self.cv_mat_kn[:] = np.nan
            
            self.u_kn = np.zeros([self.K, N_max])         # u_kn[k,n] is the reduced potential energy without umbrella restraints of snapshot n of umbrella simulation k (only needed if T is not constant)
            self.u_kln = np.zeros([self.K, self.K, N_max])     # u_kln[k,l,n] is the reduced potential energy of snapshot n from umbrella simulation k evaluated at umbrella l
            self.b_kln = np.zeros([self.K, self.K, N_max])     # b_kln[k,l,n] is the static bias of snapshot n (from umbrella simulation k) evaluated at umbrella l
            ...
            for k in range(self.K):
                ...
                cv_details = np.asarray( [self.dict_umbr_sets[which_set].umbr_harm_locs[image_in_set], self.dict_umbr_sets[which_set].umbr_harm_ks[image_in_set]] )
                ...
                indices = timeseries.subsample_correlated_data(self.cv_mat_kn[k, 0:len(cv_vals)], g=self.g_k[k])
                ...
                if self.verbose:
                    print(f"set {which_set} image {image_in_set}: stat_ineff={self.g_k[k]} for {self.N_k[k]} frames")
            ...
            for i in range(len(self.umbr_sets_keys)):
                ...
                if this_umbr_set.umbr_static_file != None:
                    ...
                    counter = 0
                    for k in range(self.K):
                        self.b_kln[k, locs, 0:self.N_k[k]] = ext_bias_vals[counter:counter+self.N_k[k]]
                        counter += self.N_k[k]
        
            hist, bins = np.histogram(self.cv_mat_kn, bins=nbins, range=(self.cv_min,self.cv_max), density=False)
            cv_min, cv_max = check_histogram(hist, bins)
            
            #------ Set Up For Final MBAR --------------------------------------------------------------
            # Evaluate reduced energies in all umbrellas
            if self.verbose: print("Evaluating reduced potential energies...")
            # Set zero of u_kn -- this is arbitrary.
            self.u_kn -= self.u_kn.min() # arbitrary up to a constant
            eval_reduced_pot_energies(self.N_k, self.u_kln, self.u_kn, self.beta_k, self.cv_mat_kn, self.restraint_k, self.b_kln)
            ...
            if self.verbose: print("Creating FES Object...")
            ...
            if self.verbose: print("Generating FES...")
            fes.generate_fes(self.u_kn, cv_n, fes_type="histogram", histogram_parameters=histogram_parameters)
            ...
            text   += f"# provided T={} K, resulitng in KT={self.kT} {self.units}"
